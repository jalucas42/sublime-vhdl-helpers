...
class VhdlPortDeclToPortAssignCopyCommand(sublime_plugin.TextCommand):
	def run(self, edit):
		selection = self.view.sel()
		for region in selection:
			region_text = self.view.substr(region)
			...
			for line in lines_orig:
				...
				# Convert the ports to signals.
				# FIXME: Insert semicolon always, even if port doesn't have one.
				line = re.sub(r'^\s*([a-z0-9_]+\s*):\s*(?:IN|OUT|INOUT|BUFFER)\s+(.+)$', r'  \1=> \1,', line, flags=re.IGNORECASE);
...
class VhdlPortDeclToPortAssignBlankCommand(sublime_plugin.TextCommand):
	def run(self, edit):
		selection = self.view.sel()
		for region in selection:
			region_text = self.view.substr(region)
			...
			for line in lines_orig:
				...
				# Convert the ports to signals.
				# FIXME: Insert semicolon always, even if port doesn't have one.
				line = re.sub(r'^\s*([a-z0-9_]+\s*):\s*(?:IN|OUT|INOUT|BUFFER)\s+(.+)$', r'  \1=> ,', line, flags=re.IGNORECASE);
